...
def calcular_score_governanca():
    ...
    scores.append(5 if st.session_state.comites else 2)
    scores.append(map_auditoria[st.session_state.auditoria])
    scores.append(1 if st.session_state.ressalvas else 5)
    scores.append(map_compliance[st.session_state.compliance])
    scores.append(map_litigios[st.session_state.litigios])
    scores.append(1 if st.session_state.renegociacao else 5)
    scores.append(1 if st.session_state.midia_negativa else 5)
...
def calcular_score_operacional():
    ...
    scores.append(5 if st.session_state.politica_formalizada else 2)
...
def calcular_score_lastro_projeto():
    ...
    elif avanco_obra >= 70: score_avanco = 4
    elif avanco_obra >= 40: score_avanco = 3
    elif avanco_obra >= 10: score_avanco = 2
    else: score_avanco = 1
...
# ==============================================================================
# FUNÇÕES DE MODELAGEM FINANCEIRA
# ==============================================================================
    ...
    try:
        ...
        for mes in range(1, prazo + 1):
            ...
            if amortizacao_tipo == 'Price':
                ...
                principal = saldo_devedor / prazo
            else:
                pmt = npf.pmt(taxa_am, prazo - mes + 1, -saldo_atual)
                principal = pmt - juros
            principal = min(principal, saldo_atual)
            fluxo.append({"Mês": mes, "Juros Recebidos": juros, "Amortização Recebida": principal, "Pagamento Total": juros + principal, "Saldo Devedor": saldo_atual - principal})
...
def gerar_fluxo_projeto(ss):
    ...
    try:
        ...
        df_unidades['estoque'] = pd.to_numeric(df_unidades['estoque'])
        df_unidades['area'] = pd.to_numeric(df_unidades['area'])
        df_unidades['preco_m2'] = pd.to_numeric(df_unidades['preco_m2'])
...
with tab2:
    ...
    if st.session_state.tipo_lastro == 'Desenvolvimento Imobiliário (Risco de Projeto)':
        ...
        with st.expander("Fator 3: Risco de Execução (Peso: 35%)"):
            ...
            st.selectbox("Aderência ao cronograma físico da obra:", ["Adiantado ou no prazo", "Atraso leve (< 3 meses)", "Atraso significativo (3-6 meses)", "Atraso severo (> 6 meses)"], key='cronograma')
            st.selectbox("Aderência ao orçamento da obra:", ["Dentro do orçamento", "Estouro leve (<5%)", "Estouro moderado (5-10%)", "Estouro severo (>10%)"], key='orcamento')
            st.selectbox("Suficiência do Fundo de Obras para custo remanescente:", ["Suficiente com margem (>110%)", "Suficiente (100-110%)", "Insuficiente (<100%)"], key='fundo_obras')
        ...
        with st.expander("Fator 1: Qualidade da Carteira (Peso: 40%)", expanded=True):
            ...
            st.selectbox("Qualidade da política de crédito que originou a carteira:", ["Robusta e bem documentada (score, DTI, etc.)", "Padrão de mercado", "Frouxa, ad-hoc ou desconhecida"], key='origem')
        with st.expander("Fator 2: Performance Histórica (Peso: 40%)"):
            st.number_input("Índice de inadimplência da carteira (> 90 dias) (%)", key='inadimplencia')
            st.selectbox("Análise de safras (vintage) mostra um comportamento:", ["Estável ou melhorando", "Com leve deterioração", "Com deterioração clara e preocupante"], key='vintage')
        with st.expander("Fator 3: Concentração (Peso: 20%)"):
            st.number_input("Concentração da carteira nos 5 maiores devedores (%)", key='concentracao_top5')
...
with tab4:
    ...
    with st.expander("Fator 1: Conflitos de Interesse (Peso: 50%)", expanded=True):
        st.selectbox("Nível de independência entre Originador, Securitizadora e Gestor", ["Totalmente independentes", "Partes relacionadas com mitigação de conflitos", "Mesmo grupo econômico com alto potencial de conflito"], key='independencia')
        st.checkbox("O originador/cedente retém a cota subordinada ou outra forma de risco relevante?", key='retencao_risco')
        st.selectbox("Histórico de decisões em assembleias do estruturador/originador", ["Alinhado aos interesses dos investidores", "Decisões mistas, alguns waivers aprovados", "Histórico de decisões que beneficiam o devedor"], key='historico_decisoes')
    with st.expander("Fator 2: Qualidade dos Prestadores de Serviço (Peso: 30%)"):
        st.selectbox("Reputação e experiência do Agente Fiduciário", ["Alta, com histórico de proatividade", "Média, cumpre o papel protocolar", "Baixa, passivo ou com histórico negativo"], key='ag_fiduciario')
        st.selectbox("Reputação e experiência da Securitizadora", ["Alta, experiente e com bom histórico", "Média, com histórico misto", "Nova ou com histórico negativo"], key='securitizadora')
    ...
    with st.expander("Fator 3: Robustez Contratual e Transparência (Peso: 20%)"):
        st.selectbox("Qualidade e rigidez dos Covenants da operação", ["Fortes, objetivos e com gatilhos claros", "Padrão, com alguma subjetividade", "Fracos, subjetivos ou fáceis de contornar"], key='covenants')
        st.selectbox("Qualidade dos pareceres jurídicos (true sale, etc.)", ["Abrangentes e conclusivos (escritório 1ª linha)", "Padrão, cumprem requisitos formais", "Limitados ou com ressalvas"], key='pareceres')
        st.selectbox("Qualidade e frequência dos relatórios de acompanhamento", ["Alta, detalhados e frequentes", "Média, cumprem o mínimo regulatório", "Baixa, informações inconsistentes ou atrasadas"], key='relatorios')
